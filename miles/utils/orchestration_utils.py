...
def init_orchestration_script(args: Namespace, *, disposer: AsyncExitStack) -> ActorHandle | None:
    event_logger_checkpoint.restore(args)
    configure_logger(args, source=SimpleProcessIdentity(component="main"))
    maybe_start_periodic_pyspy_dump()
    init_tracking(args)
    disposer.callback(finish_tracking)
    worker_manager = launch_worker_manager(args)
    disposer.push_async_callback(shutdown_worker_manager, worker_manager)
