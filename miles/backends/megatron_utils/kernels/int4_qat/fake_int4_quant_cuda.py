...
_AITER_B_SHUF_CACHE = {}  # aiter shuffled B cache
...
def _matmul(self, other):
    if isinstance(other, torch.Tensor) and other.dim() == 2 and other.dtype == torch.bfloat16:
        ...
        if k1 >= 2048 and k2 >= 2048:
            ...
            # AITER fast path (bpreshuffle + splitK=1 measured fastest for 8192^3)
            if _AITER_AVAILABLE:
                if key not in _AITER_B_SHUF_CACHE:
                    # aiter expects B as [N, K] row-major, i.e. original weight transposed into contiguous RM
                    aiter_b = other.t().contiguous()
                    try:
                        shuffled = shuffle_weight(aiter_b, layout=(16, 16), use_int4=False)
                        _AITER_B_SHUF_CACHE[key] = shuffled
                    except Exception:
                        # shuffling not supported for this shape; fall back to plain transposed cache
                        _AITER_B_SHUF_CACHE[key] = aiter_b
                aiter_b = _AITER_B_SHUF_CACHE[key]
                use_bpreshuffle = getattr(aiter_b, "is_shuffled", False)
                ...
                try:
                    gemm_a16w16_asm(self, aiter_b, out, bias=None, splitK=4, kernelName=None, bpreshuffle=use_bpreshuffle)
                    return out
                except Exception:
