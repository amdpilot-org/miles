...
async def test_a_failed_broadcast_closes_the_update_window_without_readying_cells():
    """A failed trainer broadcast releases its controller window without accepting partial weights."""
    from miles.ray.placement_group import update_weights

    order: list[str] = []
    inference_controller = _OrderRecordingInferenceController(order)
    actor_model = MagicMock(update_weights=AsyncMock(side_effect=RuntimeError("rollout engine died")))
...
@pytest.mark.asyncio
async def test_a_cancelled_broadcast_waits_for_the_rpc_to_finish_before_closing_the_window():
    """Cancellation keeps the update window locked until the trainer broadcast is terminal."""
    from miles.ray.placement_group import update_weights
    ...
    async def _update_weights(*, info: object, rollout_id: int | None = None) -> int:
        started.set()
        await finish.wait()
        order.append("trainer_update_weights_finished")
        return 11
    ...
    assert order == ["start_update_weights"]

    finish.set()
    with pytest.raises(asyncio.CancelledError):
        await updating

    assert order == ["start_update_weights", "trainer_update_weights_finished", "end_update_weights"]
    [end_kwargs] = [kwargs for name, _args, kwargs in inference_controller.calls if name == "end_update_weights"]
    assert end_kwargs == {"snapshot_cell_id_to_hashes": {}}
